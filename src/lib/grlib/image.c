...
//
//*****************************************************************************
static bool    PixelTransparentDraw(const tContext* pContext,
                                    int32_t         i32X,
                                    int32_t         i32Y,
                                    int32_t         i32X0,
                                    int32_t         i32Count,
    ...
    switch (i32BPP & 0xFF)
    {
        ...
        case 1:
        {
            ...
            for (i32Index = 0; i32Index < i32NumBytes;)
            {
                ...
                for (i32Off = 0; i32Index < i32NumBytes;)
                {
                    ...
                    //
                    i32Count = NumLeadingZeros(((pui8Data[i32Index] ^ ui32Mask) << (24 + i32Bit)));
                ...
                {
                    i32Draw = ((i32Off + i32On) > i32Len) ? (i32X + i32Len) : (i32X + i32Off + i32On);
                    DpyLineDrawH(pContext->psDisplay,
                                 i32X + i32Off,
                                 i32Draw - 1,
                                 i32Y,
                                 *(uint32_t*)(pui8Palette + (ui32Transparent ? 0 : 4)));
        ...
        case 4:
        {
            ...
            for (i32Index = 1; i32Index < i32Count; i32Index++)
            {
                ...
                // Read the next pixel.
                //
                ui8Pixel = (pui8Data[(i32Index + i32Bit) / 2] >> (i32X0 ? 0 : 4)) & 0x0F;
        ...
        case 8:
        {
            ...
            if (!bSkip && i32Len)
            {
                DpyPixelDrawMultiple(pContext->psDisplay, i32X + i32Start, i32Y, i32X0, i32Len, i32BPP, &pui8Data[i32Start], pui8Palette);
...
//
//*****************************************************************************
static void InternalImageDraw(const tContext* pContext,
                              ...
                              bool            bTransparent)
{
    ...
    //
    if ((i32X > pContext->sClipRegion.i16XMax) || ((i32X + i32Width - 1) < pContext->sClipRegion.i16XMin) ||
        (i32Y > pContext->sClipRegion.i16YMax) || ((i32Y + i32Height - 1) < pContext->sClipRegion.i16YMin))
    {
        return;
    }
        ...
        while (i32Height && ui32Count)
        {
            ...
            while (ui32Size || !(ui32Byte & (1 << (ui32Bits - 1))))
            {
                ...
                while (ui32Size && (ui32Idx != sizeof(g_pui8Dictionary)))
                {
                    ...
                    // Copy this byte.
                    //
                    g_pui8Dictionary[ui32Idx] = g_pui8Dictionary[(ui32Idx + ui32Match) % sizeof(g_pui8Dictionary)];
                ...
                if ((ui32Idx == sizeof(g_pui8Dictionary)) || !ui32Count)
                {
                    ...
                    //
                    for (ui32Idx = 0; (ui32Idx < sizeof(g_pui8Dictionary)) && i32Height;)
                    {
                        //
                        // Compute the number of pixels that remain in the
                        // dictionary buffer.
                        //
                        ui32Num = ((sizeof(g_pui8Dictionary) - ui32Idx) * 8) / i32BPP;

                        //
                        // See if any of the pixels in the dictionary buffer
                        // are within the clipping region.
                        //
                        if ((i32Y >= pContext->sClipRegion.i16YMin) && ((i32X1 + ui32Num) >= i32X0) && (i32X1 <= i32X2))
                        {
                            //
                        ...
                        //
                        if (((((i32Width - i32X1) * i32BPP) + 7) / 8) > (sizeof(g_pui8Dictionary) - ui32Idx))
                        {
                            //
                            ...
                            // dictionary buffer.
                            //
                            i32X1 += (((sizeof(g_pui8Dictionary) - ui32Idx) * 8) / i32BPP);
...
//
//*****************************************************************************
void GrTransparentImageDraw(const tContext* pContext, const uint8_t* pui8Image, int32_t i32X, int32_t i32Y, uint32_t ui32Transparent)
{
    InternalImageDraw(pContext, pui8Image, i32X, i32Y, ui32Transparent, true);
}
...
//
//*****************************************************************************
void GrImageDraw(const tContext* pContext, const uint8_t* pui8Image, int32_t i32X, int32_t i32Y)
{
