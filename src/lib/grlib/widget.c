...
uint32_t __attribute__((naked)) WidgetMutexGet(uint8_t* pi8Mutex)
{
    uint32_t ui32Ret;
...
__asm uint32_t WidgetMutexGet(uint8_t* pi8Mutex)
{
    mov r1, #1 ldrexb r2, [r0] cmp r2, #0 it eq strexbeq r2, r1, [r0] mov r0, r2 bx lr
}
#endif
//
...
//
//*****************************************************************************
static bool WidgetIsInTree(tWidget* psWidget, tWidget* psFind)
{
    tWidget* psTemp;
...
void WidgetRemove(tWidget* psWidget)
{
    ...
    else
    {
        ...
        //
        for (psTemp = psWidget->psParent->psChild; psTemp->psNext != psWidget; psTemp = psTemp->psNext)
        {
        }
...
uint32_t WidgetMessageSendPreOrder(tWidget* psWidget, uint32_t ui32Message, uint32_t ui32Param1, uint32_t ui32Param2, bool bStopOnSuccess)
{
    ...
    //
    ui32Ret = psWidget->pfnMsgProc(psWidget, ui32Message, ui32Param1, ui32Param2);
    ...
    for (psTemp = psWidget->psChild; psTemp != psWidget;)
    {
        ...
        //
        ui32Ret = psTemp->pfnMsgProc(psTemp, ui32Message, ui32Param1, ui32Param2);
        if ((ui32Ret != 0) && bStopOnSuccess)
        {
            return (ui32Ret);
        }
...
uint32_t WidgetMessageSendPostOrder(tWidget* psWidget, uint32_t ui32Message, uint32_t ui32Param1, uint32_t ui32Param2, bool bStopOnSuccess)
{
    ...
    //
    if ((ui32Message == WIDGET_MSG_PTR_MOVE) || (ui32Message == WIDGET_MSG_PTR_UP))
    {
        //
        ...
        // messages.
        //
        ui32Ret = g_psPointerWidget->pfnMsgProc(g_psPointerWidget, ui32Message, ui32Param1, ui32Param2);
...
//
//*****************************************************************************
int32_t WidgetMessageQueueAdd(tWidget* psWidget,
                              uint32_t ui32Message,
                              uint32_t ui32Param1,
                              uint32_t ui32Param2,
                              bool     bPostOrder,
                              bool     bStopOnSuccess)
{
    ...
    if (ui32Message == WIDGET_MSG_PTR_MOVE)
    {
        ...
        if (g_ui32MQRead != g_ui32MQWrite)
        {
            ...
            //
            ui32Owned = (g_ui32MQWrite == 0) ? (QUEUE_SIZE - 1) : (g_ui32MQWrite - 1);
    ...
    //
    g_psMQ[g_ui32MQWrite].ui32Flags   = ((bPostOrder ? MQ_FLAG_POST_ORDER : 0) | (bStopOnSuccess ? MQ_FLAG_STOP_ON_SUCCESS : 0));
...
void WidgetMessageQueueProcess(void)
{
    ...
    while (g_ui32MQRead != g_ui32MQWrite)
    {
        ...
        if (ui32Flags & MQ_FLAG_POST_ORDER)
        {
            ...
            //
            WidgetMessageSendPostOrder(psWidget,
                                       ui32Message,
                                       ui32Param1,
                                       ui32Param2,
                                       ((ui32Flags & MQ_FLAG_STOP_ON_SUCCESS) ? true : false));
        }
        else
        {
            //
            ...
            //
            WidgetMessageSendPreOrder(psWidget,
                                      ui32Message,
                                      ui32Param1,
                                      ui32Param2,
                                      ((ui32Flags & MQ_FLAG_STOP_ON_SUCCESS) ? true : false));
...
int32_t WidgetPointerMessage(uint32_t ui32Message, int32_t i32X, int32_t i32Y)
{
    ...
    //
    return (WidgetMessageQueueAdd(WIDGET_ROOT, ui32Message, i32X, i32Y, true, true));
}
