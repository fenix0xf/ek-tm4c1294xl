...
typedef struct
{
    ...
    //
    void (*pfnPixelDraw)(void* pvDisplayData, int32_t i32X, int32_t i32Y, uint32_t ui32Value);
    ...
    //
    void (*pfnLineDrawV)(void* pvDisplayData, int32_t i32X, int32_t i32Y1, int32_t i32Y2, uint32_t ui32Value);
...
typedef struct
{
    ...
    //
    uint32_t (*pfnFontBlockCodepointsGet)(uint8_t* pui8FontId, uint16_t ui16BlockIndex, uint32_t* pui32Start);
} tFontAccessFuncs;
...
typedef struct
{
    ...
    //
    uint32_t (*pfnMapChar)(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
} tCodePointMap;
...
//
//*****************************************************************************
typedef void (*tStringRenderer)(const struct _tContext*, const char*, int32_t, int32_t, int32_t, bool);
...
//
//*****************************************************************************
#define GrContextDpyWidthGet(psContext)  (DpyWidthGet((psContext)->psDisplay))
...
//
//*****************************************************************************
#define GrContextDpyHeightGet(psContext) (DpyHeightGet((psContext)->psDisplay))
...
//
//*****************************************************************************
#define GrFontBaselineGet(psFont) ((psFont)->ui8Baseline)
...
//
//*****************************************************************************
#define GrFontHeightGet(psFont)   ((psFont)->ui8Height)
...
//
//*****************************************************************************
#define GrFontMaxWidthGet(psFont) ((psFont)->ui8MaxWidth)
#endif
...
//
//*****************************************************************************
#define GrImageColorsGet(pui8Image)              (((uint8_t*)pui8Image)[5] + 1)
...
//
//*****************************************************************************
#define GrImageHeightGet(pui8Image)              (*(uint16_t*)(pui8Image + 3))
...
//
//*****************************************************************************
#define GrImageWidthGet(pui8Image)               (*(uint16_t*)(pui8Image + 1))
...
//
//*****************************************************************************
#define GrOffScreen1BPPSize(i32Width, i32Height) (5 + (((i32Width + 7) / 8) * i32Height))
...
//
//*****************************************************************************
#define GrOffScreen4BPPSize(i32Width, i32Height) (6 + (16 * 3) + (((i32Width + 1) / 2) * i32Height))
...
//
//*****************************************************************************
#define GrOffScreen8BPPSize(i32Width, i32Height) (6 + (256 * 3) + (i32Width * i32Height))
...
#define GrPixelDraw(psContext, i32X, i32Y)                                                                                 \
    do {                                                                                                                   \
        const tContext* pC = psContext;                                                                                    \
        if ((i32X >= pC->sClipRegion.i16XMin) && (i32X <= pC->sClipRegion.i16XMax) && (i32Y >= pC->sClipRegion.i16YMin) && \
...
//
//*****************************************************************************
#define GrStringBaselineGet(psContext) (GrFontBaselineGet((psContext)->psFont))
...
#define GrStringDrawCentered(psContext, pcString, i32Length, i32X, i32Y, bOpaque) \
    do {                                                                          \
        ...
        GrStringDraw(pC,                                                          \
                     pcStr,                                                       \
                     i32Length,                                                   \
                     (i32X) - (GrStringWidthGet(pC, pcStr, i32Length) / 2),       \
                     (i32Y) - (int32_t)(GrFontBaselineGet(pC->psFont) / 2),       \
                     bOpaque);                                                    \
    } while (0)
...
//
//*****************************************************************************
#define GrStringHeightGet(psContext)   (GrFontHeightGet((psContext)->psFont))
...
//
//*****************************************************************************
#define GrStringMaxWidthGet(psContext) (GrFontMaxWidthGet((psContext)->psFont))
...
//
//*****************************************************************************
#define DpyColorTranslate(psDisplay, ui32Value) ((psDisplay)->pfnColorTranslate((psDisplay)->pvDisplayData, ui32Value))
...
//
//*****************************************************************************
#define DpyHeightGet(psDisplay) ((psDisplay)->ui16Height)
...
#define DpyLineDrawH(psDisplay, i32X1, i32X2, i32Y, ui32Value)              \
    do {                                                                    \
        const tDisplay* pD = psDisplay;                                     \
        pD->pfnLineDrawH(pD->pvDisplayData, i32X1, i32X2, i32Y, ui32Value); \
    } while (0)
...
#define DpyLineDrawV(psDisplay, i32X, i32Y1, i32Y2, ui32Value)              \
    do {                                                                    \
        const tDisplay* pD = psDisplay;                                     \
        pD->pfnLineDrawV(pD->pvDisplayData, i32X, i32Y1, i32Y2, ui32Value); \
    } while (0)
...
//
//*****************************************************************************
#define DpyPixelDrawMultiple(psDisplay, i32X, i32Y, i32X0, i32Count, i32BPP, pui8Data, pui8Palette)              \
    do {                                                                                                         \
        const tDisplay* pD = psDisplay;                                                                          \
        pD->pfnPixelDrawMultiple(pD->pvDisplayData, i32X, i32Y, i32X0, i32Count, i32BPP, pui8Data, pui8Palette); \
    } while (0)
...
//
//*****************************************************************************
#define DpyWidthGet(psDisplay) ((int16_t)(psDisplay)->ui16Width)
...
#define GrRectContainsPoint(psRect, i32X, i32Y)                                                                                         \
    ((((i32X) >= (psRect)->i16XMin) && ((i32X) <= (psRect)->i16XMax) && ((i32Y) >= (psRect)->i16YMin) && ((i32Y) <= (psRect)->i16YMax)) \
         ? 1                                                                                                                            \
         : 0)
...
#endif
extern void GrCircleDraw(const tContext* psContext, int32_t i32X, int32_t i32Y, int32_t i32Radius);
extern void GrCircleFill(const tContext* psContext, int32_t i32X, int32_t i32Y, int32_t i32Radius);
...
extern void GrImageDraw(const tContext* psContext, const uint8_t* pui8Image, int32_t i32X, int32_t i32Y);
extern void GrTransparentImageDraw(const tContext* psContext,
                                   const uint8_t*  pui8Image,
                                   int32_t         i32X,
                                   int32_t         i32Y,
                                   uint32_t        ui32Transparent);
extern void GrLineDraw(const tContext* psContext, int32_t i32X1, int32_t i32Y1, int32_t i32X2, int32_t i32Y2);
extern void GrLineDrawH(const tContext* psContext, int32_t i32X1, int32_t i32X2, int32_t i32Y);
extern void GrLineDrawV(const tContext* psContext, int32_t i32X, int32_t i32Y1, int32_t i32Y2);
extern void GrOffScreen1BPPInit(tDisplay* psDisplay, uint8_t* pui8Image, int32_t i32Width, int32_t i32Height);
extern void GrOffScreen4BPPInit(tDisplay* psDisplay, uint8_t* pui8Image, int32_t i32Width, int32_t i32Height);
extern void GrOffScreen4BPPPaletteSet(tDisplay* psDisplay, uint32_t* pui32Palette, uint32_t ui32Offset, uint32_t ui32Count);
extern void GrOffScreen8BPPInit(tDisplay* psDisplay, uint8_t* pui8Image, int32_t i32Width, int32_t i32Height);
extern void GrOffScreen8BPPPaletteSet(tDisplay* psDisplay, uint32_t* pui32Palette, uint32_t ui32Offset, uint32_t ui32Count);
...
extern void GrStringDraw(const tContext* psContext, const char* pcString, int32_t i32Length, int32_t i32X, int32_t i32Y, uint32_t bOpaque);
extern int32_t GrStringWidthGet(const tContext* psContext, const char* pcString, int32_t i32Length);
...
extern int32_t GrRectIntersectGet(tRectangle* psRect1, tRectangle* psRect2, tRectangle* psIntersect);
...
//
//*****************************************************************************
extern void     GrFontInfoGet(const tFont* psFont, uint8_t* pui8Format, uint8_t* pui8MaxWidth, uint8_t* pui8Height, uint8_t* pui8Baseline);
...
extern const uint8_t* GrFontGlyphDataGet(const tFont* psFont, uint32_t ui32CodePoint, uint8_t* pui8Width);
extern uint16_t       GrFontCodepageGet(const tFont* psFont);
extern uint16_t       GrFontNumBlocksGet(const tFont* psFont);
extern uint32_t       GrFontBlockCodepointsGet(const tFont* psFont, uint16_t ui16BlockIndex, uint32_t* pui32Start);
...
//
//*****************************************************************************
void GrFontGlyphRender(const tContext* psContext, const uint8_t* pui8Data, int32_t i32X, int32_t i32Y, bool bCompressed, bool bOpaque);
void GrDefaultStringRenderer(const tContext* psContext, const char* pcString, int32_t i32Length, int32_t i32X, int32_t i32Y, bool bOpaque);
...
//
//*****************************************************************************
extern void     GrCodepageMapTableSet(tContext* psContext, tCodePointMap* pCodePointMapTable, uint8_t ui8NumMaps);
extern int32_t  GrStringCodepageSet(tContext* psContext, uint16_t ui16Codepage);
extern uint32_t GrStringRendererSet(tContext* psContext, tStringRenderer pfnRenderer);
extern uint32_t GrStringNumGlyphsGet(const tContext* psContext, const char* pcString);
extern uint32_t GrStringNextCharGet(const tContext* psContext, const char* pcString, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapUnicode_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapUTF8_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapUTF16LE_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapUTF16BE_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapISO8859_1_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapISO8859_2_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapISO8859_3_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapISO8859_4_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapISO8859_5_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapISO8859_6_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapISO8859_7_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapISO8859_8_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapISO8859_9_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapISO8859_10_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapISO8859_11_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapISO8859_13_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapISO8859_14_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapISO8859_15_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapISO8859_16_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapWIN1250_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapWIN1251_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapWIN1252_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapWIN1253_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
extern uint32_t GrMapWIN1254_Unicode(const char* pcSrcChar, uint32_t ui32Count, uint32_t* pui32Skip);
