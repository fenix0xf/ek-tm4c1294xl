...
#define NUM_DEFAULT_CODEPOINT_MAPS (sizeof(g_psDefaultCodePointMapTable) / sizeof(tCodePointMap))
...
void GrContextInit(tContext* psContext, const tDisplay* psDisplay)
{
    ...
    psContext->sClipRegion.i16XMax = (int16_t)(psDisplay->ui16Width - 1);
    psContext->sClipRegion.i16YMax = (int16_t)(psDisplay->ui16Height - 1);
...
void GrContextClipRegionSet(tContext* psContext, tRectangle* pRect)
{
    ...
    //
    psContext->sClipRegion.i16XMin = ((pRect->i16XMin < 0) ? 0 : ((pRect->i16XMin >= ui32W) ? (ui32W - 1) : pRect->i16XMin));
    psContext->sClipRegion.i16YMin = ((pRect->i16YMin < 0) ? 0 : ((pRect->i16YMin >= ui32H) ? (ui32H - 1) : pRect->i16YMin));
    psContext->sClipRegion.i16XMax = ((pRect->i16XMax < 0) ? 0 : ((pRect->i16XMax >= ui32W) ? (ui32W - 1) : pRect->i16XMax));
    psContext->sClipRegion.i16YMax = ((pRect->i16YMax < 0) ? 0 : ((pRect->i16YMax >= ui32H) ? (ui32H - 1) : pRect->i16YMax));
}
