...
#include "lib/grlib/radiobutton.h"
...
//
//*****************************************************************************
static void RadioButtonPaint(tWidget* psWidget, uint32_t bClick)
{
    ...
    i32Y = (psWidget->sPosition.i16YMin + ((psWidget->sPosition.i16YMax - psWidget->sPosition.i16YMin) / 2));
    if (!bClick)
    {
        GrContextForegroundSet(&sCtx, pRadio->ui32OutlineColor);
    ...
    if ((pRadio->ui16Style & (RB_STYLE_TEXT | RB_STYLE_IMG)) && !bClick)
    {
        ...
        if (pRadio->ui16Style & RB_STYLE_IMG)
        {
            ...
            //
            if (GrImageHeightGet(pRadio->pui8Image) > (sCtx.sClipRegion.i16YMax - sCtx.sClipRegion.i16YMin))
            {
            ...
            {
                i32Y = (sCtx.sClipRegion.i16YMin +
                        ((sCtx.sClipRegion.i16YMax - sCtx.sClipRegion.i16YMin - GrImageHeightGet(pRadio->pui8Image) + 1) / 2));
            }
            ...
            //
            GrImageDraw(&sCtx, pRadio->pui8Image, sCtx.sClipRegion.i16XMin, i32Y);
        }
        ...
        if (pRadio->ui16Style & RB_STYLE_TEXT)
        {
            ...
            // it takes less than the Y extent.
            //
            if (GrFontHeightGet(pRadio->psFont) > (sCtx.sClipRegion.i16YMax - sCtx.sClipRegion.i16YMin))
            {
                i32Y = sCtx.sClipRegion.i16YMin;
            }
            else
            {
                i32Y = (sCtx.sClipRegion.i16YMin +
                        ((sCtx.sClipRegion.i16YMax - sCtx.sClipRegion.i16YMin - GrFontHeightGet(pRadio->psFont) + 1) / 2));
            }
            ...
            GrContextForegroundSet(&sCtx, pRadio->ui32TextColor);
            GrContextBackgroundSet(&sCtx, pRadio->ui32FillColor);
            GrStringDraw(&sCtx, pRadio->pcText, -1, sCtx.sClipRegion.i16XMin, i32Y, pRadio->ui16Style & RB_STYLE_TEXT_OPAQUE);
        }
    }
}
...
//
//*****************************************************************************
static int32_t RadioButtonClick(tWidget* psWidget, uint32_t ui32Msg, int32_t i32X, int32_t i32Y)
{
        ...
        (i32Y <= psWidget->sPosition.i16YMax))
    {
        ...
        // selected.
        //
        if ((ui32Msg == WIDGET_MSG_PTR_UP) && !(pRadio->ui16Style & RB_STYLE_SELECTED))
        {
            //
            // Loop through the siblings of this radio button widget.
            //
            for (pSibling = psWidget->psParent->psChild; pSibling; pSibling = pSibling->psNext)
            {
                //
                // Skip this widget if it is not a radio button widget, or if
                // it is the original radio button widget.
                //
                if ((pSibling == psWidget) || (pSibling->pfnMsgProc != RadioButtonMsgProc))
                {
                    continue;
                }
...
//
//*****************************************************************************
int32_t RadioButtonMsgProc(tWidget* psWidget, uint32_t ui32Msg, uint32_t ui32Param1, uint32_t ui32Param2)
{
    //
    // Check the arguments.
    //
    ...
    switch (ui32Msg)
    {
        ...
        case WIDGET_MSG_PTR_UP:
        {
            ...
            //
            return (RadioButtonClick(psWidget, ui32Msg, ui32Param1, ui32Param2));
        }
...
//
//*****************************************************************************
void RadioButtonInit(tRadioButtonWidget* psWidget,
