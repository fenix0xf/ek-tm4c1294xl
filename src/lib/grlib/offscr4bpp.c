...
//
//*****************************************************************************
static uint32_t GrOffScreen4BPPColorTranslate(void* pvDisplayData, uint32_t ui32Value)
{
    ...
    for (ui32Idx = 0; ui32Idx < 16; ui32Idx++, pui8Palette += 3)
    {
        ...
        //
        ui32Diff = (((pui8Palette[2] - ui32R) * (pui8Palette[2] - ui32R)) + ((pui8Palette[1] - ui32G) * (pui8Palette[1] - ui32G)) +
...
//
//*****************************************************************************
static void GrOffScreen4BPPPixelDraw(void* pvDisplayData, int32_t i32X, int32_t i32Y, uint32_t ui32Value)
{
    uint8_t* pui8Data;
...
//
//*****************************************************************************
static void GrOffScreen4BPPPixelDrawMultiple(void*          pvDisplayData,
                                             int32_t        i32X,
                                             int32_t        i32Y,
                                             int32_t        i32X0,
                                             int32_t        i32Count,
        ...
        case 1:
        {
            ...
            while (i32Count)
            {
                ...
                for (; (i32X0 < 8) && i32Count; i32X0++, i32Count--)
                {
                    ...
                    //
                    *pui8Ptr = ((*pui8Ptr & ~(0xf << i32X)) | (((uint32_t*)pui8Palette)[(ui32Byte >> (7 - i32X0)) & 1] << i32X));
                    if (i32X ^= 4)
                    {
                        pui8Ptr++;
                    }
            ...
            switch (i32X0 & 1)
            {
                case 0:
                    while (i32Count)
                    {
                        ...
                        //
                        ui32Byte = GrOffScreen4BPPColorTranslate(pvDisplayData, ui32Byte);
...
//
//*****************************************************************************
static void GrOffScreen4BPPLineDrawH(void* pvDisplayData, int32_t i32X1, int32_t i32X2, int32_t i32Y, uint32_t ui32Value)
{
    uint8_t* pui8Data;
    int32_t  i32BytesPerRow;
    ...
    // a time).
    //
    ui32Value = ((ui32Value << 28) | (ui32Value << 24) | (ui32Value << 20) | (ui32Value << 16) | (ui32Value << 12) | (ui32Value << 8) |
...
//
//*****************************************************************************
static void GrOffScreen4BPPLineDrawV(void* pvDisplayData, int32_t i32X, int32_t i32Y1, int32_t i32Y2, uint32_t ui32Value)
{
    uint8_t* pui8Data;
    int32_t  i32BytesPerRow;
...
//
//*****************************************************************************
static void GrOffScreen4BPPRectFill(void* pvDisplayData, const tRectangle* pRect, uint32_t ui32Value)
{
    ...
    // starting pixel.
    //
    pui8Data += ((i32BytesPerRow * pRect->i16YMin) + (pRect->i16XMin / 2) + 6 + (16 * 3));
    ...
    if (i32X & 1)
    {
        ...
        //
        for (i32Y = pRect->i16YMin, pui8Column = pui8Data; i32Y <= pRect->i16YMax; i32Y++, pui8Column += i32BytesPerRow)
        {
    ...
    if (i32X == pRect->i16XMax)
    {
        ...
        for (i32Y = pRect->i16YMin; i32Y <= pRect->i16YMax; i32Y++, pui8Data += i32BytesPerRow)
        {
            *pui8Data = (*pui8Data & 0x0f) | ui32Value;
        }
    }
...
//
//*****************************************************************************
static void GrOffScreen4BPPFlush(void* pvDisplayData)
{
    //
    // Check the arguments.
    //
