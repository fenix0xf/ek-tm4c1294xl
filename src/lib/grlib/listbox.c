...
//
//*****************************************************************************
static void ListBoxPaint(tWidget* psWidget)
{
    ...
    // strings to draw.
    //
    while ((sLineRect.i16YMin < sWidgetRect.i16YMax) && (ui16String < pListBox->ui16Populated))
    {
        //
        ...
        GrContextBackgroundSet(
            &sCtx,
            ((ui16String == pListBox->i16Selected) ? pListBox->ui32SelectedBackgroundColor : pListBox->ui32BackgroundColor));
        GrContextForegroundSet(&sCtx, ((ui16String == pListBox->i16Selected) ? pListBox->ui32SelectedTextColor : pListBox->ui32TextColor));

        //
        // Draw the text.
        //
        GrStringDraw(&sCtx, pListBox->ppcText[ui16String], -1, sLineRect.i16XMin, sLineRect.i16YMin, 1);
...
//
//*****************************************************************************
static int32_t ListBoxPointer(tListBoxWidget* pListBox, uint32_t ui32Msg, int32_t i32X, int32_t i32Y)
{
    ...
    switch (ui32Msg)
    {
        ...
        case WIDGET_MSG_PTR_UP:
        {
            ...
            //
            if ((pListBox->ui16Scrolled == 0) && !(pListBox->ui32Style & LISTBOX_STYLE_LOCKED) &&
                GrRectContainsPoint(&(pListBox->sBase.sPosition), i32X, i32Y))
            {
                ...
                //
                i32LineNum = (i32Y - (int32_t)pListBox->sBase.sPosition.i16YMin) / GrFontHeightGet(pListBox->psFont);
                ...
                //
                i32Entry = ((int32_t)pListBox->ui16StartEntry + i32LineNum) % pListBox->ui16MaxEntries;
                ...
                {
                    (pListBox->pfnOnChange)((tWidget*)pListBox, pListBox->i16Selected);
                }
            }
        ...
        case WIDGET_MSG_PTR_MOVE:
        {
            ...
            if (abs(i32LineNum) >= GrFontHeightGet(pListBox->psFont))
            {
                ...
                //
                i32Visible =
                    (pListBox->sBase.sPosition.i16YMax - pListBox->sBase.sPosition.i16YMin) / (int32_t)GrFontHeightGet(pListBox->psFont);
                ...
                //
                i32Scroll = i32LineNum / (int32_t)GrFontHeightGet(pListBox->psFont);
                ...
                //
                i32MaxDown = (pListBox->ui16StartEntry >= pListBox->ui16OldestEntry)
                               ? (pListBox->ui16OldestEntry - pListBox->ui16StartEntry)
                               : ((pListBox->ui16OldestEntry - pListBox->ui16StartEntry) - pListBox->ui16MaxEntries);

                //
                // What is the farthest we could scroll upwards?  Note - this
                // will be a positive number.
                //
                i32MaxUp = ((int32_t)pListBox->ui16Populated - i32Visible) + i32MaxDown;
                ...
                if (i32Scroll)
                {
                    ...
                    //
                    pListBox->i32PointerY -= (i32Scroll * GrFontHeightGet(pListBox->psFont));
...
int32_t ListBoxMsgProc(tWidget* psWidget, uint32_t ui32Msg, uint32_t ui32Param1, uint32_t ui32Param2)
{
    ...
    switch (ui32Msg)
    {
        ...
        case WIDGET_MSG_PTR_DOWN:
        case WIDGET_MSG_PTR_UP:
        case WIDGET_MSG_PTR_MOVE: return (ListBoxPointer(pListBox, ui32Msg, (int32_t)ui32Param1, (int32_t)ui32Param2));
...
//
//*****************************************************************************
void ListBoxInit(tListBoxWidget* psWidget,
