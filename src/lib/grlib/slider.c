...
//
//*****************************************************************************
static int16_t SliderValueToPosition(tSliderWidget* pSlider, int32_t i32Value)
{
    uint16_t ui16Size;
    int32_t  i32Range;
    int32_t  i32Pos;
    ...
    if (pSlider->ui32Style & SL_STYLE_VERTICAL)
    {
        ...
        //
        ui16Size = (pSlider->sBase.sPosition.i16YMax - pSlider->sBase.sPosition.i16YMin) + 1;
    }
    else
    {
        //
        ...
        //
        ui16Size = (pSlider->sBase.sPosition.i16XMax - pSlider->sBase.sPosition.i16XMin) + 1;
    }
...
//
//*****************************************************************************
static int32_t SliderPositionToValue(tSliderWidget* pSlider, int16_t i16Pos)
{
    int16_t i16Max;
    int16_t i16Min;
    int32_t i32Value;
    ...
    //
    i32Value = ((int32_t)i16Pos * ((pSlider->i32Max - pSlider->i32Min) + 1)) / (int32_t)((i16Max - i16Min) + 1);
...
//
//*****************************************************************************
static void SliderPaint(tWidget* psWidget, tRectangle* psDirty)
{
    ...
    //
    bIntersect = GrRectIntersectGet(psDirty, &(pSlider->sBase.sPosition), &sClipRect);
    ...
    if (bIntersect)
    {
        ...
        if (pSlider->ui32Style & SL_STYLE_TEXT)
        {
            ...
            GrContextForegroundSet(&sCtx, pSlider->ui32TextColor);
            GrContextBackgroundSet(&sCtx, pSlider->ui32FillColor);
            GrStringDrawCentered(&sCtx, pSlider->pcText, -1, i32X, i32Y, pSlider->ui32Style & SL_STYLE_TEXT_OPAQUE);
        }
    }
    ...
    if (bIntersect)
    {
        ...
        if (pSlider->ui32Style & SL_STYLE_BACKG_TEXT)
        {
            ...
            GrContextForegroundSet(&sCtx, pSlider->ui32BackgroundTextColor);
            GrContextBackgroundSet(&sCtx, pSlider->ui32BackgroundFillColor);
            GrStringDrawCentered(&sCtx, pSlider->pcText, -1, i32X, i32Y, pSlider->ui32Style & SL_STYLE_BACKG_TEXT_OPAQUE);
        }
    }
}
...
//
//*****************************************************************************
static int32_t SliderClick(tWidget* psWidget, uint32_t ui32Msg, int32_t i32X, int32_t i32Y)
{
    tSliderWidget* pSlider;
    ...
    if ((ui32Msg == WIDGET_MSG_PTR_MOVE) ||
        ((ui32Msg == WIDGET_MSG_PTR_DOWN) && (i32X >= psWidget->sPosition.i16XMin) && (i32X <= psWidget->sPosition.i16XMax) &&
         (i32Y >= psWidget->sPosition.i16YMin) && (i32Y <= psWidget->sPosition.i16YMax)))
    {
        //
        // Map the pointer position to a slider value.
        //
        i32NewVal = SliderPositionToValue(pSlider, (pSlider->ui32Style & SL_STYLE_VERTICAL) ? i32Y : i32X);
...
//
//*****************************************************************************
int32_t SliderMsgProc(tWidget* psWidget, uint32_t ui32Msg, uint32_t ui32Param1, uint32_t ui32Param2)
{
    //
    // Check the arguments.
    //
