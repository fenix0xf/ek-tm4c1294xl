...
//
//*****************************************************************************
static void ImageButtonPaint(tWidget* psWidget)
{
    ...
    if (psPush->ui32Style & IB_STYLE_FILL)
    {
        ...
        //
        GrContextForegroundSet(&sCtx, ((psPush->ui32Style & IB_STYLE_PRESSED) ? psPush->ui32PressedColor : psPush->ui32BackgroundColor));
    ...
    GrContextBackgroundSet(&sCtx, ((psPush->ui32Style & IB_STYLE_PRESSED) ? psPush->ui32PressedColor : psPush->ui32BackgroundColor));
    ...
    if (!(psPush->ui32Style & IB_STYLE_IMAGE_OFF))
    {
        ...
        //
        pui8Image = ((psPush->ui32Style & IB_STYLE_PRESSED) ? psPush->pui8PressImage : psPush->pui8Image);
...
//
//*****************************************************************************
static int32_t ImageButtonClick(tWidget* psWidget, uint32_t ui32Msg, int32_t i32X, int32_t i32Y)
{
    tImageButtonWidget* psPush;
    ...
    if (ui32Msg == WIDGET_MSG_PTR_UP)
    {
        ...
        //
        if (GrRectContainsPoint(&psWidget->sPosition, i32X, i32Y) && (psPush->ui32Style & IB_STYLE_RELEASE_NOTIFY) && psPush->pfnOnClick)
        {
    ...
    if (GrRectContainsPoint(&psWidget->sPosition, i32X, i32Y))
    {
        ...
        if (psPush->pfnOnClick)
        {
            ...
            //
            if ((ui32Msg == WIDGET_MSG_PTR_DOWN) && !(psPush->ui32Style & IB_STYLE_RELEASE_NOTIFY))
            {
                psPush->pfnOnClick(psWidget);
            }
            ...
            if (psPush->ui32Style & IB_STYLE_AUTO_REPEAT)
            {
                ...
                else if (ui32Msg == WIDGET_MSG_PTR_MOVE)
                {
                    ...
                    // call the callback.
                    //
                    if ((psPush->ui32AutoRepeatCount >= psPush->ui16AutoRepeatDelay) &&
                        (((psPush->ui32AutoRepeatCount - psPush->ui16AutoRepeatDelay) % psPush->ui16AutoRepeatRate) == 0))
                    {
                        psPush->pfnOnClick(psWidget);
                    }
                }
...
//
//*****************************************************************************
int32_t ImageButtonMsgProc(tWidget* psWidget, uint32_t ui32Msg, uint32_t ui32Param1, uint32_t ui32Param2)
{
    //
    // Check the arguments.
    //
    ...
    switch (ui32Msg)
    {
        ...
        case WIDGET_MSG_PTR_UP:
        {
            ...
            //
            return (ImageButtonClick(psWidget, ui32Msg, ui32Param1, ui32Param2));
        }
...
//
//*****************************************************************************
void ImageButtonInit(tImageButtonWidget* psWidget,
