...
//
//*****************************************************************************
static void ContainerPaint(tWidget* psWidget)
{
    ...
    if (pContainer->ui32Style & CTR_STYLE_TEXT)
    {
        ...
        {
            i32X1 = (psWidget->sPosition.i16XMin + ((psWidget->sPosition.i16XMax - psWidget->sPosition.i16XMin + 1 - i32X2 - 8) / 2));
        }
        else
        {
        ...
        //
        GrStringDraw(&sCtx, pContainer->pcText, -1, i32X1 + 4, psWidget->sPosition.i16YMin, pContainer->ui32Style & CTR_STYLE_TEXT_OPAQUE);
        ...
        if (pContainer->ui32Style & CTR_STYLE_OUTLINE)
        {
            ...
            //
            i32Y = (psWidget->sPosition.i16YMin + (GrFontBaselineGet(pContainer->psFont) / 2));
            ...
            GrLineDraw(&sCtx, psWidget->sPosition.i16XMin, i32Y, psWidget->sPosition.i16XMin, psWidget->sPosition.i16YMax);
            GrLineDraw(&sCtx,
                       psWidget->sPosition.i16XMin,
                       psWidget->sPosition.i16YMax,
                       psWidget->sPosition.i16XMax,
                       psWidget->sPosition.i16YMax);
            GrLineDraw(&sCtx, psWidget->sPosition.i16XMax, psWidget->sPosition.i16YMax, psWidget->sPosition.i16XMax, i32Y);
...
//
//*****************************************************************************
int32_t ContainerMsgProc(tWidget* psWidget, uint32_t ui32Msg, uint32_t ui32Param1, uint32_t ui32Param2)
{
    //
    // Check the arguments.
    //
...
//
//*****************************************************************************
void ContainerInit(tContainerWidget* psWidget, const tDisplay* psDisplay, int32_t i32X, int32_t i32Y, int32_t i32Width, int32_t i32Height)
{
    uint32_t ui32Idx;
