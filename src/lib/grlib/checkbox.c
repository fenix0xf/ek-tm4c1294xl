...
//
//*****************************************************************************
static void CheckBoxPaint(tWidget* psWidget, bool bClick)
{
    ...
    i16Rect.i16YMin =
        (psWidget->sPosition.i16YMin + ((psWidget->sPosition.i16YMax - psWidget->sPosition.i16YMin - pCheck->ui16BoxSize + 1) / 2));
    ...
    //
    GrLineDraw(&sCtx, i16Rect.i16XMin + 1, i16Rect.i16YMin + 1, i16Rect.i16XMax - 1, i16Rect.i16YMax - 1);
    GrLineDraw(&sCtx, i16Rect.i16XMin + 1, i16Rect.i16YMax - 1, i16Rect.i16XMax - 1, i16Rect.i16YMin + 1);
    ...
    if ((pCheck->ui16Style & (CB_STYLE_TEXT | CB_STYLE_IMG)) && !bClick)
    {
        ...
        if (pCheck->ui16Style & CB_STYLE_IMG)
        {
            ...
            //
            if (GrImageHeightGet(pCheck->pui8Image) > (sCtx.sClipRegion.i16YMax - sCtx.sClipRegion.i16YMin))
            {
            ...
            {
                i32Y = (sCtx.sClipRegion.i16YMin +
                        ((sCtx.sClipRegion.i16YMax - sCtx.sClipRegion.i16YMin - GrImageHeightGet(pCheck->pui8Image) + 1) / 2));
            }
            ...
            //
            GrImageDraw(&sCtx, pCheck->pui8Image, sCtx.sClipRegion.i16XMin, i32Y);
        }
        ...
        if (pCheck->ui16Style & CB_STYLE_TEXT)
        {
            ...
            // it takes less than the Y extent.
            //
            if (GrFontHeightGet(pCheck->psFont) > (sCtx.sClipRegion.i16YMax - sCtx.sClipRegion.i16YMin))
            {
                i32Y = sCtx.sClipRegion.i16YMin;
            }
            else
            {
                i32Y = (sCtx.sClipRegion.i16YMin +
                        ((sCtx.sClipRegion.i16YMax - sCtx.sClipRegion.i16YMin - GrFontHeightGet(pCheck->psFont) + 1) / 2));
            }
            ...
            GrContextForegroundSet(&sCtx, pCheck->ui32TextColor);
            GrContextBackgroundSet(&sCtx, pCheck->ui32FillColor);
            GrStringDraw(&sCtx, pCheck->pcText, -1, sCtx.sClipRegion.i16XMin, i32Y, pCheck->ui16Style & CB_STYLE_TEXT_OPAQUE);
        }
    }
}
...
//
//*****************************************************************************
static int32_t CheckBoxClick(tWidget* psWidget, uint32_t ui32Msg, int32_t i32X, int32_t i32Y)
{
    tCheckBoxWidget* pCheck;
    ...
    //
    if ((i32X >= psWidget->sPosition.i16XMin) && (i32X <= psWidget->sPosition.i16XMax) && (i32Y >= psWidget->sPosition.i16YMin) &&
        (i32Y <= psWidget->sPosition.i16YMax))
    {
        ...
        if (ui32Msg == WIDGET_MSG_PTR_UP)
        {
            ...
            {
                pCheck->pfnOnChange(psWidget, pCheck->ui16Style & CB_STYLE_SELECTED);
            }
        }
...
//
//*****************************************************************************
int32_t CheckBoxMsgProc(tWidget* psWidget, uint32_t ui32Msg, uint32_t ui32Param1, uint32_t ui32Param2)
{
    //
    // Check the arguments.
    //
...
//
//*****************************************************************************
void CheckBoxInit(tCheckBoxWidget* psWidget, const tDisplay* psDisplay, int32_t i32X, int32_t i32Y, int32_t i32Width, int32_t i32Height)
{
    uint32_t ui32Idx;
