...
#include "lib/grlib/pushbutton.h"
...
//
//*****************************************************************************
static void RectangularButtonPaint(tWidget* psWidget)
{
    ...
    if (pPush->ui32Style & PB_STYLE_FILL)
    {
        ...
        //
        GrContextForegroundSet(&sCtx, ((pPush->ui32Style & PB_STYLE_PRESSED) ? pPush->ui32PressFillColor : pPush->ui32FillColor));
    ...
    if (pPush->ui32Style & (PB_STYLE_TEXT | PB_STYLE_IMG))
    {
        ...
        //
        i32X = (psWidget->sPosition.i16XMin + ((psWidget->sPosition.i16XMax - psWidget->sPosition.i16XMin + 1) / 2));
        i32Y = (psWidget->sPosition.i16YMin + ((psWidget->sPosition.i16YMax - psWidget->sPosition.i16YMin + 1) / 2));
        ...
        if (pPush->ui32Style & PB_STYLE_IMG)
        {
            ...
            GrContextBackgroundSet(&sCtx, ((pPush->ui32Style & PB_STYLE_PRESSED) ? pPush->ui32PressFillColor : pPush->ui32FillColor));

            //
            // Get the image to be drawn.
            //
            pui8Image = (((pPush->ui32Style & PB_STYLE_PRESSED) && pPush->pui8PressImage) ? pPush->pui8PressImage : pPush->pui8Image);

            //
            // Draw the image centered in the push button.
            //
            GrImageDraw(&sCtx, pui8Image, i32X - (GrImageWidthGet(pui8Image) / 2), i32Y - (GrImageHeightGet(pui8Image) / 2));
        }
        ...
        if (pPush->ui32Style & PB_STYLE_TEXT)
        {
            ...
            GrContextForegroundSet(&sCtx, pPush->ui32TextColor);
            GrContextBackgroundSet(&sCtx, ((pPush->ui32Style & PB_STYLE_PRESSED) ? pPush->ui32PressFillColor : pPush->ui32FillColor));
            GrStringDrawCentered(&sCtx, pPush->pcText, -1, i32X, i32Y, pPush->ui32Style & PB_STYLE_TEXT_OPAQUE);
        }
    }
}
...
//
//*****************************************************************************
static int32_t RectangularButtonClick(tWidget* psWidget, uint32_t ui32Msg, int32_t i32X, int32_t i32Y)
{
    tPushButtonWidget* pPush;
    ...
    if (ui32Msg == WIDGET_MSG_PTR_UP)
    {
        ...
        //
        if ((pPush->ui32Style & PB_STYLE_FILL) || ((pPush->ui32Style & PB_STYLE_IMG) && pPush->pui8PressImage))
        {
        ...
        //
        if (GrRectContainsPoint(&psWidget->sPosition, i32X, i32Y) && (pPush->ui32Style & PB_STYLE_RELEASE_NOTIFY) && pPush->pfnOnClick)
        {
    ...
    if (GrRectContainsPoint(&psWidget->sPosition, i32X, i32Y))
    {
        ...
        if (pPush->pfnOnClick)
        {
            ...
            //
            if ((ui32Msg == WIDGET_MSG_PTR_DOWN) && !(pPush->ui32Style & PB_STYLE_RELEASE_NOTIFY))
            {
                pPush->pfnOnClick(psWidget);
            }
            ...
            if (pPush->ui32Style & PB_STYLE_AUTO_REPEAT)
            {
                ...
                else if (ui32Msg == WIDGET_MSG_PTR_MOVE)
                {
                    ...
                    // call the callback.
                    //
                    if ((pPush->ui32AutoRepeatCount >= pPush->ui16AutoRepeatDelay) &&
                        (((pPush->ui32AutoRepeatCount - pPush->ui16AutoRepeatDelay) % pPush->ui16AutoRepeatRate) == 0))
                    {
                        pPush->pfnOnClick(psWidget);
                    }
                }
...
int32_t RectangularButtonMsgProc(tWidget* psWidget, uint32_t ui32Msg, uint32_t ui32Param1, uint32_t ui32Param2)
{
    ...
    switch (ui32Msg)
    {
        ...
        case WIDGET_MSG_PTR_UP:
        {
            ...
            //
            return (RectangularButtonClick(psWidget, ui32Msg, ui32Param1, ui32Param2));
        }
...
//
//*****************************************************************************
void RectangularButtonInit(tPushButtonWidget* psWidget,
...
//
//*****************************************************************************
static void CircularButtonPaint(tWidget* psWidget)
{
    const uint8_t*     pui8Image;
    tPushButtonWidget* pPush;
    tContext           sCtx;
...
//
//*****************************************************************************
static int32_t CircularButtonClick(tWidget* psWidget, uint32_t ui32Msg, int32_t i32X, int32_t i32Y)
{
    tPushButtonWidget* pPush;
    ...
    //
    if ((((i32X - i32Xc) * (i32X - i32Xc)) + ((i32Y - i32Yc) * (i32Y - i32Yc))) <= (i32R * i32R))
    {
        //
        // See if this is a pointer down message.
        //
        ...
        if (pPush->pfnOnClick)
        {
            ...
            //
            if (((ui32Msg == WIDGET_MSG_PTR_DOWN) && !(pPush->ui32Style & PB_STYLE_RELEASE_NOTIFY)) ||
                ((ui32Msg == WIDGET_MSG_PTR_UP) && (pPush->ui32Style & PB_STYLE_RELEASE_NOTIFY)))
            {
                pPush->pfnOnClick(psWidget);
            }
...
int32_t CircularButtonMsgProc(tWidget* psWidget, uint32_t ui32Msg, uint32_t ui32Param1, uint32_t ui32Param2)
{
    ...
    switch (ui32Msg)
    {
        ...
        case WIDGET_MSG_PTR_UP:
        {
            ...
            // Handle the pointer request, returning the appropriate value.
            //
            return (CircularButtonClick(psWidget, ui32Msg, ui32Param1, ui32Param2));
        }
