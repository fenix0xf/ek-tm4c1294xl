...
int32_t GrStringWidthGet(const tContext* pContext, const char* pcString, int32_t i32Length)
{
...
int32_t GrStringWidthGet(const tContext* pContext, const char* pcString, int32_t i32Length)
{
    ...
    while (ui32Count)
    {
        ...
        //
        ui32Char = GrStringNextCharGet(pContext, pcString, ui32Count, &ui32Skip);
        ...
        if (!pui8Data)
        {
            ...
            //
            pui8Data = GrFontGlyphDataGet(pContext->psFont, ABSENT_CHAR_REPLACEMENT, &ui8Width);
            ...
            if (!pui8Data)
            {
                ...
                //
                pui8Data = GrFontGlyphDataGet(pContext->psFont, 0x20, &ui8Width);
                ...
                if (!pui8Data)
                {
                    ...
                    //
                    GrFontInfoGet(pContext->psFont, &ui8Format, &ui8Width, &ui8Height, &ui8Baseline);
                }
            }
        }
...
//
void GrStringDraw(const tContext* pContext, const char* pcString, int32_t i32Length, int32_t i32X, int32_t i32Y, uint32_t bOpaque)
{
    ...
    while (*pcString && i32Length--)
    {
        ...
        for (i32Idx = 2, i32X0 = 0, i32Bit = 0, i32Y0 = 0; i32Idx < pui8Data[0];)
        {
            ...
            //
            if ((i16Con.psFont->ui8Format & ~FONT_EX_MARKER) == FONT_FMT_UNCOMPRESSED)
            {
                //
            ...
            while (i32Off)
            {
                ...
                if ((i32Off > 1) && ((i32X0 + 1) < pui8Data[1]))
                {
                    ...
                    // row.
                    //
                    i32Count = (((i32X0 + i32Off) > pui8Data[1]) ? pui8Data[1] - i32X0 : i32Off);
                    ...
                    // pixels.
                    //
                    if (((i32Y + i32Y0) >= i16Con.sClipRegion.i16YMin) && bOpaque)
                    {
                        i16Con.ui32Foreground = pContext->ui32Background;
                        GrLineDrawH(&i16Con, i32X + i32X0, i32X + i32X0 + i32Count - 1, i32Y + i32Y0);
                    }
                ...
                else
                {
                    ...
                    // it.
                    //
                    if (((i32X + i32X0) >= i16Con.sClipRegion.i16XMin) && ((i32X + i32X0) <= i16Con.sClipRegion.i16XMax) &&
                        ((i32Y + i32Y0) >= i16Con.sClipRegion.i16YMin) && bOpaque)
                    {
                        DpyPixelDraw(pContext->psDisplay, i32X + i32X0, i32Y + i32Y0, pContext->ui32Background);
                    }
            ...
            while (i32On)
            {
                ...
                if ((i32On > 1) && ((i32X0 + 1) < pui8Data[1]))
                {
                    ...
                    // row.
                    //
                    i32Count = (((i32X0 + i32On) > pui8Data[1]) ? pui8Data[1] - i32X0 : i32On);
                ...
                else
                {
                    ...
                    {
                        DpyPixelDraw(pContext->psDisplay, i32X + i32X0, i32Y + i32Y0, pContext->ui32Foreground);
                    }
...
void GrStringDraw(const tContext* pContext, const char* pcString, int32_t i32Length, int32_t i32X, int32_t i32Y, uint32_t bOpaque)
{
    ...
    //
    pContext->pfnStringRenderer(pContext, pcString, i32Length, i32X, i32Y, bOpaque);
}
...
//
//*****************************************************************************
void GrDefaultStringRenderer(const tContext* pContext, const char* pcString, int32_t i32Length, int32_t i32X, int32_t i32Y, bool bOpaque)
{
    ...
    //
    GrFontInfoGet(pContext->psFont, &ui8Format, &ui8MaxWidth, &ui8Height, &ui8Baseline);

    //
    // If the string is completely outside the clipping region, don't even
    // start rendering it.
    //
    if ((i32Y > pContext->sClipRegion.i16YMax) || ((i32Y + ui8Height) < pContext->sClipRegion.i16YMin))
    {
        return;
    }
    ...
    while (ui32Count)
    {
        ...
        if (!pui8Data)
        {
            ...
            if (!pui8Data)
            {
                ...
                //
                pui8Data = GrFontGlyphDataGet(pContext->psFont, ' ', &ui8Width);
            }
        }
        ...
        if (pui8Data)
        {
            GrFontGlyphRender(pContext, pui8Data, i32X, i32Y, (ui8Format & FONT_FMT_PIXEL_RLE) ? true : false, bOpaque);
            i32X += ui8Width;
        }
        else
        {
...
//!
//*****************************************************************************
uint32_t GrStringNextCharGet(const tContext* pContext, const char* pcString, uint32_t ui32Count, uint32_t* pui32Skip)
{
    ASSERT(pContext);
    ASSERT(pcString);
    ASSERT(pui32Skip);
    ...
    if (pContext->pCodePointMapTable)
    {
        ...
        //
        return (pContext->pCodePointMapTable[pContext->ui8CodePointMap].pfnMapChar(pcString, ui32Count, pui32Skip));
    }
    else
    {
        //
...
//
//*****************************************************************************
void GrFontGlyphRender(const tContext* pContext, const uint8_t* pui8Data, int32_t i32X, int32_t i32Y, bool bCompressed, bool bOpaque)
{
    int32_t i32Idx, i32X0, i32Y0, i32Count, i32Off, i32On, i32Bit;
    ...
    for (i32Idx = 2, i32X0 = 0, i32Bit = 0, i32Y0 = 0; i32Idx < pui8Data[0];)
    {
        ...
        while (i32Off)
        {
            ...
            if ((i32Off > 1) && ((i32X0 + 1) < pui8Data[1]))
            {
                ...
                // pixels.
                //
                if (((i32Y + i32Y0) >= pContext->sClipRegion.i16YMin) && bOpaque)
                {
                    ...
                    if ((i32X + i32X0 + i32Count - 1) > pContext->sClipRegion.i16XMax)
                    {
                    ...
                    DpyLineDrawH(pContext->psDisplay, i32ClipX1, i32ClipX2, i32Y + i32Y0, pContext->ui32Background);
                }
            ...
            else
            {
                ...
                // it.
                //
                if (((i32X + i32X0) >= pContext->sClipRegion.i16XMin) && ((i32X + i32X0) <= pContext->sClipRegion.i16XMax) &&
                    ((i32Y + i32Y0) >= pContext->sClipRegion.i16YMin) && bOpaque)
                {
                    DpyPixelDraw(pContext->psDisplay, i32X + i32X0, i32Y + i32Y0, pContext->ui32Background);
                }
        ...
        while (i32On)
        {
            ...
            if ((i32On > 1) && ((i32X0 + 1) < pui8Data[1]))
            {
                ...
                if ((i32Y + i32Y0) >= pContext->sClipRegion.i16YMin)
                {
                    ...
                    DpyLineDrawH(pContext->psDisplay, i32ClipX1, i32ClipX2, i32Y + i32Y0, pContext->ui32Foreground);
                }
...
//
//*****************************************************************************
void GrFontInfoGet(const tFont* psFont, uint8_t* pui8Format, uint8_t* pui8MaxWidth, uint8_t* pui8Height, uint8_t* pui8Baseline)
{
    //
    ...
    if (psFont->ui8Format & FONT_FMT_WRAPPED)
    {
        ...
        psFontWrapper->pFuncs->pfnFontInfoGet(psFontWrapper->pui8FontId, pui8Format, pui8MaxWidth, pui8Height, pui8Baseline);
    }
    else
    {
        //
...
uint32_t GrFontBaselineGet(const tFont* psFont)
{
    ...
    else
    {
        ...
        pWrap->pFuncs->pfnFontInfoGet(pWrap->pui8FontId, &ui8Format, &ui8Width, &ui8Height, &ui8Baseline);
...
//
//*****************************************************************************
static const uint8_t* FontGlyphDataGet(const tFont* psFont, uint32_t ui32CodePoint, uint8_t* pui8Width)
{
    const uint8_t*  pui8Glyphs;
    const uint16_t* pui16Offset;
...
//
//*****************************************************************************
static const uint8_t* FontWideGlyphDataGet(const tFontWide* psFont, uint32_t ui32CodePoint, uint8_t* pui8Width)
{
    const uint8_t* pui8Data;
    tFontBlock*    pBlock;
    ...
    for (ui32Loop = 0; ui32Loop < psFont->ui16NumBlocks; ui32Loop++)
    {
        ...
        if ((ui32CodePoint >= pBlock[ui32Loop].ui32StartCodepoint) &&
            (ui32CodePoint < (pBlock[ui32Loop].ui32StartCodepoint + pBlock[ui32Loop].ui32NumCodepoints)))
        {
            //
    ...
    //
    pui32OffsetTable = (uint32_t*)((uint8_t*)psFont + pBlock[ui32Loop].ui32GlyphTableOffset);
    ui32Offset       = pui32OffsetTable[ui32CodePoint - pBlock[ui32Loop].ui32StartCodepoint];
...
//
//*****************************************************************************
const uint8_t* GrFontGlyphDataGet(const tFont* psFont, uint32_t ui32CodePoint, uint8_t* pui8Width)
{
    ASSERT(psFont);
    ASSERT(pui8Width);
    ...
    if (psFont->ui8Format == FONT_FMT_WRAPPED)
    {
        ...
        //
        psFontWrapper = (tFontWrapper*)psFont;
        return (psFontWrapper->pFuncs->pfnFontGlyphDataGet(psFontWrapper->pui8FontId, ui32CodePoint, pui8Width));
    }
    else if (psFont->ui8Format & FONT_WIDE_MARKER)
    {
        ...
        //
        return (FontWideGlyphDataGet((const tFontWide*)psFont, ui32CodePoint, pui8Width));
    }
    else
    {
        //
...
uint16_t GrFontCodepageGet(const tFont* psFont)
{
    ...
    else if (psFont->ui8Format & FONT_FMT_WRAPPED)
    {
        ...
        return (((tFontWrapper*)psFont)->pFuncs->pfnFontCodepageGet(((tFontWrapper*)psFont)->pui8FontId));
    }
    else
    {
        //
...
static int32_t UpdateContextCharMapping(tContext* pContext)
{
    ...
    {
        if ((pContext->pCodePointMapTable[ui32Loop].ui16SrcCodepage == pContext->ui16Codepage) &&
            (pContext->pCodePointMapTable[ui32Loop].ui16FontCodepage == ui16FontCodepage))
        {
            //
...
uint16_t GrFontNumBlocksGet(const tFont* psFont)
{
    ...
    else if (psFont->ui8Format & FONT_FMT_WRAPPED)
    {
        ...
        return (((tFontWrapper*)psFont)->pFuncs->pfnFontNumBlocksGet(((tFontWrapper*)psFont)->pui8FontId));
    }
    else
    {
        //
...
//
//*****************************************************************************
uint32_t GrFontBlockCodepointsGet(const tFont* psFont, uint16_t ui16BlockIndex, uint32_t* pui32Start)
{
    ASSERT(psFont);
    ASSERT(pui32Start);
    ...
    else if (psFont->ui8Format & FONT_FMT_WRAPPED)
    {
        ...
        return (
            ((tFontWrapper*)psFont)->pFuncs->pfnFontBlockCodepointsGet(((tFontWrapper*)psFont)->pui8FontId, ui16BlockIndex, pui32Start));
    }
    else
    {
        //
...
//
//*****************************************************************************
void GrCodepageMapTableSet(tContext* pContext, tCodePointMap* pCodePointMapTable, uint8_t ui8NumMaps)
{
    ASSERT(pContext);
...
void                   GrStringTableSet(const void* pvTable)
{
    ...
    //
    g_pui32StringTable = (uint32_t*)(g_pui16LanguageTable + g_ui16NumLanguages);

    //
    // Save a pointer to the String Data.
    //
    g_pui8StringData = (uint8_t*)(g_pui32StringTable + (g_ui16NumStrings * g_ui16NumLanguages));
}
...
uint32_t GrStringGet(int32_t i32Index, char* pcData, uint32_t ui32Size)
{
    ...
    //
    ui32SubCode[i32Pos] = g_pui32StringTable[(g_ui16Language * g_ui16NumStrings) + i32Index];
    ...
    if (SC_GET_LEN(ui32SubCode[i32Pos]))
    {
        ...
        while (i32Pos < 16)
        {
            ...
            ui32SubCode[i32Pos] = g_pui32StringTable[(g_ui16Language * g_ui16NumStrings) + i32Idx];
    ...
    while (i32Pos >= 0)
    {
        ...
        else if (ui32Offset & SC_FLAG_COMPRESSED)
        {
            ...
            //
            if (!SC_GET_LEN(ui32SubCode[i32Pos]) && SC_GET_INDEX(ui32SubCode[i32Pos]))
            {
            ...
            else if (!SC_GET_LEN(ui32SubCode[0]) && SC_GET_INDEX(ui32SubCode[0]))
            {
        ...
        else if (i32Pos)
        {
            ...
            {
                pcData[i32Idx + i32Buf] = g_pui8StringData[ui32Offset + i32Buf];
            }
