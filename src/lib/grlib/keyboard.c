...
#include "lib/grlib/widget.h"
#include "lib/grlib/keyboard.h"
...
//
//*****************************************************************************
static void ButtonPaintText(tWidget* psWidget, const tKeyText* psKey)
{
    ...
    sRect.i16XMax = sRect.i16XMin - 3 + ((ui32Range * (uint32_t)psKey->ui16Width) / 10000);
    ...
    sRect.i16YMax = sRect.i16YMin - 3 + ((ui32Range * (uint32_t)psKey->ui16Height) / 10000);
    ...
    if (psKeyboard->ui32Style & KEYBOARD_STYLE_FILL)
    {
        ...
        GrContextForegroundSet(&sCtx,
                               ((psKeyboard->ui32Flags & FLAG_KEY_PRESSED) ? psKeyboard->ui32PressFillColor : psKeyboard->ui32FillColor));
    ...
    GrStringDrawCentered(&sCtx, (const char*)pcKeyCap, ui32Size, i32X, i32Y, psKeyboard->ui32Style & KEYBOARD_STYLE_TEXT_OPAQUE);
}
...
//
//*****************************************************************************
static void KeyboardPaint(tWidget* psWidget)
{
    int32_t          i32Key;
...
//
//*****************************************************************************
static uint32_t FindKey(tKeyboardWidget* psKeyWidget, const tKeyboard* psKeyboard, int32_t i32X, int32_t i32Y)
{
    uint32_t        ui32Key;
    uint32_t        ui32Range;
    ...
    for (ui32Key = 0; ui32Key < psKeyboard->ui16NumKeys; ui32Key++)
    {
        ...
        //
        ui32Range  = psKeyWidget->sBase.sPosition.i16XMax - psKeyWidget->sBase.sPosition.i16XMin + 1;
        ...
        //
        ui32Range  = psKeyWidget->sBase.sPosition.i16YMax - psKeyWidget->sBase.sPosition.i16YMin + 1;
        ...
        if ((i32X >= i32XMin) && (i32X <= i32XMax) && (i32Y >= i32YMin) && (i32Y <= i32YMax))
        {
            break;
        }
    }
...
//
//*****************************************************************************
static int32_t TextButtonEvent(tWidget* psWidget, uint32_t ui32Msg, int32_t i32X, int32_t i32Y)
{
    ...
    if (ui32Msg == WIDGET_MSG_PTR_UP)
    {
        ...
        //
        if ((psKeyWidget->ui32Style & KEYBOARD_STYLE_FILL) || ((psKeyWidget->ui32Style & KEYBOARD_STYLE_IMG)))
        {
            //
            ...
            if (psKeyWidget->ui32KeyPressed < psKeyboard->ui16NumKeys)
            {
                ...
                //
                ButtonPaintText(psWidget, &psKeyboard->uKeys.psKeysText[psKeyWidget->ui32KeyPressed]);
            }
        }
        ...
        //
        if ((ui32Key < psKeyboard->ui16NumKeys) && (psKeyWidget->ui32Style & KEYBOARD_STYLE_RELEASE_NOTIFY) && psKeyWidget->pfnOnEvent)
        {
            psKeyWidget->pfnOnEvent(psWidget, psKeyboard->uKeys.psKeysText[ui32Key].ui32Code, KEYBOARD_EVENT_RELEASE);
        }
    }
    ...
    if (ui32Key < psKeyboard->ui16NumKeys)
    {
        ...
        if (ui32Msg == WIDGET_MSG_PTR_DOWN)
        {
            ...
            //
            if (psKeyboard->uKeys.psKeysText[ui32Key].ui32Code == UNICODE_CUSTOM_SHIFT)
            {
            ...
            if (psKeyboard->uKeys.psKeysText[ui32Key].ui32Code == UNICODE_CUSTOM_MODE_TOG)
            {
            ...
            else if ((psKeyWidget->ui32Active == 1) && ((psKeyWidget->ui32Flags & FLAG_KEY_CAPSLOCK) == 0))
            {
                psKeyWidget->ui32Flags  &= ~FLAG_KEY_CAPSLOCK;
                psKeyWidget->ui32Active  = 0;
            ...
            if ((psKeyWidget->ui32Style & KEYBOARD_STYLE_FILL) || ((psKeyWidget->ui32Style & KEYBOARD_STYLE_IMG)))
            {
                ...
                ButtonPaintText(psWidget, &psKeyboard->uKeys.psKeysText[ui32Key]);
            }
        }
        ...
        if (psKeyWidget->pfnOnEvent)
        {
            ...
            //
            if ((ui32Msg == WIDGET_MSG_PTR_DOWN) && (psKeyWidget->ui32Style & KEYBOARD_STYLE_PRESS_NOTIFY))
            {
                psKeyWidget->pfnOnEvent(psWidget, psKeyboard->uKeys.psKeysText[ui32Key].ui32Code, KEYBOARD_EVENT_PRESS);
            }
            ...
            if (psKeyWidget->ui32Style & KEYBOARD_STYLE_AUTO_REPEAT)
            {
                ...
                //
                else if ((ui32Msg == WIDGET_MSG_PTR_MOVE) && (psKeyWidget->pfnOnEvent) &&
                         (psKeyWidget->ui32Style & KEYBOARD_STYLE_PRESS_NOTIFY))
                {
                    ...
                    // call the callback.
                    //
                    if ((psKeyWidget->ui32AutoRepeatCount >= psKeyWidget->ui16AutoRepeatDelay) &&
                        (((psKeyWidget->ui32AutoRepeatCount - psKeyWidget->ui16AutoRepeatDelay) % psKeyWidget->ui16AutoRepeatRate) == 0))
                    {
                        psKeyWidget->pfnOnEvent(psWidget, psKeyboard->uKeys.psKeysText[ui32Key].ui32Code, KEYBOARD_EVENT_PRESS);
                    }
                }
...
//
//*****************************************************************************
int32_t KeyboardMsgProc(tWidget* psWidget, uint32_t ui32Msg, uint32_t ui32Param1, uint32_t ui32Param2)
{
    tKeyboardWidget* psKeyWidget;
    ...
    switch (ui32Msg)
    {
        ...
        default:
        {
            ...
            //
            return (WidgetDefaultMsgProc(psWidget, ui32Msg, ui32Param1, ui32Param2));
        }
    }
}
