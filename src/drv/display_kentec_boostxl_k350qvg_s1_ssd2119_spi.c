...
#include "display_kentec_boostxl_k350qvg_s1_ssd2119_spi.h"
...
//
//*****************************************************************************
#if !defined(PORTRAIT) && !defined(PORTRAIT_FLIP) && !defined(LANDSCAPE) && !defined(LANDSCAPE_FLIP)
#define LANDSCAPE
#endif
...
#define MAPPED_X(x, y)  (uint16_t)(319 - (x))
#define MAPPED_Y(x, y)  (uint16_t)(239 - (y))
#endif
#ifdef PORTRAIT_FLIP
...
//
//*****************************************************************************
static inline void WriteDataSPI(uint16_t ui16Data)
{
    GPIOPinWrite(LCD_DC_BASE, LCD_DC_PIN, LCD_DC_PIN);
    GPIOPinWrite(LCD_CS_BASE, LCD_CS_PIN, 0);

    SSIDataPut(LCD_SSI_BASE, (uint8_t)(ui16Data >> 8));
    SSIDataPut(LCD_SSI_BASE, (uint8_t)ui16Data);
...
//
//*****************************************************************************
static inline void WriteCommandSPI(uint16_t ui16Data)
{
...
static void InitSPILCDInterface(uint32_t ui32SysClock)
{
    ...
    //
    SSIConfigSetExpClk(LCD_SSI_BASE, ui32SysClock, SSI_FRF_MOTO_MODE_0, SSI_MODE_MASTER, 15000000, 8);
...
//
//*****************************************************************************
static void Kentec320x240x16_SSD2119PixelDraw(void* pvDisplayData, int32_t i32X, int32_t i32Y, uint32_t ui32Value)
{
    (void)pvDisplayData;
    ...
    //
    WriteCommandSPI(SSD2119_RAM_DATA_REG);
    WriteDataSPI((uint16_t)ui32Value);
}
        ...
        case 1:
        {
            ...
            while (i32Count)
            {
                ...
                for (; (i32X0 < 8) && i32Count; i32X0++, i32Count--)
                {
                    ...
                    //
                    WriteDataSPI((uint16_t)((uint32_t*)pui8Palette)[(ui32Byte >> (7 - i32X0)) & 1]);
                }
...
//
//*****************************************************************************
static void Kentec320x240x16_SSD2119LineDrawH(void* pvDisplayData, int32_t i32X1, int32_t i32X2, int32_t i32Y, uint32_t ui32Value)
{
    (void)pvDisplayData;
...
//
//*****************************************************************************
static void Kentec320x240x16_SSD2119LineDrawV(void* pvDisplayData, int32_t i32X, int32_t i32Y1, int32_t i32Y2, uint32_t ui32Value)
{
    (void)pvDisplayData;
...
//
//*****************************************************************************
static void Kentec320x240x16_SSD2119RectFill(void* pvDisplayData, const tRectangle* pRect, uint32_t ui32Value)
{
    (void)pvDisplayData;
    ...
    WriteDataSPI(MAPPED_Y(pRect->i16XMin, pRect->i16YMin) | (MAPPED_Y(pRect->i16XMax, pRect->i16YMax) << 8));
#else
    WriteDataSPI(MAPPED_Y(pRect->i16XMax, pRect->i16YMax) | (MAPPED_Y(pRect->i16XMin, pRect->i16YMin) << 8));
#endif
    ...
    //
    for (i32Count = ((pRect->i16XMax - pRect->i16XMin + 1) * (pRect->i16YMax - pRect->i16YMin + 1)); i32Count >= 0; i32Count--)
    {
        //
        // Write the pixel value.
        //
...
//
//*****************************************************************************
static uint32_t Kentec320x240x16_SSD2119ColorTranslate(void* pvDisplayData, uint32_t ui32Value)
{
    (void)pvDisplayData;
...
//
//*****************************************************************************
const tDisplay g_sKentec320x240x16_SSD2119 = {sizeof(tDisplay),
                                              0,
